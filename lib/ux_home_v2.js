...
function ux_home_v2() {
  ...
  const checkVideosLoaded = () => {
    return new Promise((resolve) => {
      ...
      console.log(`📹 ${isMobile ? 'MOBILE' : 'DESKTOP'} mode`);
      ...
      // MOBILE: Just wait for all videos metadata (fast)
      if (isMobile) {
        let loadedCount = 0;
        ...
        const onVideoReady = (video, index) => {
          ...
          if (loadedCount === videos.length) {
            console.log('✅ All videos metadata loaded (mobile mode)');
            resolve();
          }
        };
      ...
      // DESKTOP: Wait for first_video to be ready to play, others just metadata
      const firstVideo = document.querySelector('#first_video');
      const otherVideos = Array.from(videos).filter(v => v.id !== 'first_video');
      ...
      const checkAllReady = () => {
        if (firstVideoReady && otherVideosReady) {
          console.log('✅ All videos loaded (desktop mode)');
          resolve();
        }
      };
