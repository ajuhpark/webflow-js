/** ux card stack 
 * revised on claude so the cards are fixed but text is clickable. changes z-index. 
 * this one i also set the css for sc_text_containers to 0 and set it to 1 in the js. 
...
function ux_card_stack() {
  ...
  function init() {
    ...
    const sc_text_containers = gsap.utils.toArray(".ap_grid_1.sc_1_card_text_container");
    ...
    // Set initial z-index for all text containers to 1
    gsap.set(sc_text_containers, { 
      pointerEvents: "none",
      zIndex: 1, 
      opacity:1
    });

    // Set the first text container to be clickable
    if (sc_text_containers.length) {
      gsap.set(sc_text_containers[0], { 
        pointerEvents: "auto",
        zIndex: 10
      });
    }
    ...
    sc_cards.forEach((card, index) => {
      ...
      let text_container_index = sc_text_containers[index];
      ...
      // Text container z-index animations
      if (text_container_index) {
        // Start with a low z-index and pointer-events disabled
        tl_cards.set(text_container_index, {
          zIndex: 1,
          pointerEvents: "none"
        }, 0);
        
        // When card becomes visible, increase z-index and enable pointer events
        tl_cards.to(text_container_index, {
          zIndex: 10,
          pointerEvents: "auto",
          duration: 0.01
        }, 0.5);
        
        // Before card starts to exit, decrease z-index and disable pointer events
        tl_cards.to(text_container_index, {
          zIndex: 1,
          pointerEvents: "none",
          duration: 0.01
        }, 0.5 + entranceDuration + holdDuration - 0.1);
      }
  ...
  function killAll() {
    ...
    gsap.killTweensOf(".ap_grid_1.sc_1_card_text_container");
    ...
    const textContainers = gsap.utils.toArray(".ap_grid_1.sc_1_card_text_container");
    if (textContainers.length) {
      gsap.set(textContainers, { clearProps: "all" });
      
      // Re-hide text containers when cleaning up to prevent flash
