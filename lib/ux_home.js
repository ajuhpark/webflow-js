...
function ux_home() {
  ...
  function bannerSubheaderAnimation() {
    ...
    tl_banner_marquee_arrow
    // .set(ux_home_banner_marquee_arrow, {
    //   opacity: '100'
    // })
    .to(ux_home_banner_marquee_arrow, {
      opacity: '0%'
    })
    ...
    animations.scrollTriggers.banner_marquee_arrow = ScrollTrigger.create({
      ...
      fastScrollEnd: true, 
      onLeaveBack: () => {
        // Explicitly set opacity to 0 when scrolling up past the trigger point
        gsap.set(ux_home_banner_marquee_arrow, { opacity: 0 });
      }
    });
