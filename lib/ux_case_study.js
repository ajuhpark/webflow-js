...
function ux_case_study() {
  ...
  function init() {
    ...
    // Wait for fonts and videos before starting animations
    document.fonts.ready.then(() => {
      console.log('✅ Fonts loaded');

      // Check videos before starting animations
  ...
  function killAll() {
    ...
    // Wait for videos to be ready before reinitializing
    gsap.delayedCall(0.5, () => {
      console.log('🔄 Reinitializing after resize...');

      // Wait for videos
      checkVideosLoaded().then(() => {
        console.log('✅ Videos ready after resize');
        // Small delay to ensure layout is stable
        setTimeout(() => {
          init();
        }, 50);
