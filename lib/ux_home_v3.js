...
let splitInstances = [];
let originalContent = {};
let banner_text_tl = null; // reference to the intro timeline, so we can wait for it to finish
    ...
    document.fonts.ready.then(() => {
      ...
      requestAnimationFrame(() => {
        requestAnimationFrame(() => {
          ...
          // Refresh ScrollTrigger only after the intro timeline finishes,
          // instead of a fixed real-time delay that used to fire mid-animation
          // and cause layout-thrash jank while the intro was still playing.
          if (banner_text_tl) {
            banner_text_tl.eventCallback("onComplete", () => {
              ScrollTrigger.refresh();
            ...
            // Fallback in case the timeline wasn't created for some reason
            console.warn('banner_text_tl not found - falling back to delayed refresh');
