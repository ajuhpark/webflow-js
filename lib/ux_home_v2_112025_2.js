...
function ux_home_v2() {
  ...
  function killAll() {
    ...
    gsap.delayedCall(0.1, () => {
      ...
      // Just check video metadata is available (instant with width/height set)
      checkVideosLoaded().then(() => {
        console.log('✅ Video metadata ready - reinitializing');
        init();
  ...
  // Simplified function to ensure video metadata is loaded (for dimensions)
  // Now that we have width/height attributes, this is just a safety check
  const checkVideosLoaded = () => {
    return new Promise((resolve) => {
      ...
      const onVideoReady = (video, index) => {
        loadedCount++;
        console.log(`✅ Video ${index + 1}/${videos.length} metadata ready - ID: ${video.id || 'no-id'}`);
      ...
      videos.forEach((video, index) => {
        // Video metadata loads instantly since width/height are set
  ...
  const initializeOnce = () => {
    ...
    // Wait for fonts and video metadata (fast), then start animations immediately
    document.fonts.ready.then(() => {
      ...
      checkVideosLoaded().then(() => {
        ...
        // Refresh ScrollTrigger after images fully load for perfect accuracy
        imagesLoaded(document.body, { background: true }, function() {
          console.log('✅ Images fully loaded - refreshing ScrollTrigger');
          ScrollTrigger.refresh();
        });
