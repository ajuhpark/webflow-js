...
console.log('ux_case_study_website_builder.js is working');
...
function ux_case_study() {
  ...
  const checkVideosLoaded = () => {
    return new Promise((resolve) => {
      ...
      // Handle first video - wait for it to have enough data to start playing smoothly.
      // NOTE: previously waited for 'canplaythrough' (readyState >= 3, HAVE_FUTURE_DATA),
      // which asks the browser to estimate it can play the ENTIRE video without buffering.
      // That's overkill for a looping background video and can take several seconds,
      // especially on slower connections - it was blocking the whole page reveal.
      // 'loadeddata' (readyState >= 2, HAVE_CURRENT_DATA) fires as soon as the first
      // frame is available, which is enough to start playback smoothly.
      if (firstVideo) {
        ...
        // Check if video already has enough data (readyState >= 2 means HAVE_CURRENT_DATA)
        if (firstVideo.readyState >= 2) {
          onFirstVideoReady();
        } else {
          // Wait for loadeddata event - fires once the first frame is available
          firstVideo.addEventListener('loadeddata', onFirstVideoReady, { once: true });
  ...
  function init() {
    console.log('ux_case_study_website_builder.js init works');
    ...
    // Run all three readiness checks in parallel instead of chaining them
    // sequentially - total wait becomes the slowest of the three instead
    // of the sum of all three (they don't depend on each other to start).
    const fontsReady = document.fonts.ready.then(() => {
      console.log('✅ Fonts loaded');
    });

    const videosReady = checkVideosLoaded().then(() => {
    ...
    Promise.all([fontsReady, videosReady, imagesReady]).then(() => {
  ...
  function cs_intro_anim() {
    ...
    // Store highlighter elements content before splitting
    originalContent.highlighters = [];
    let highlighterElements = document.querySelectorAll('.ux_text_highlighter_deco');
    highlighterElements.forEach((elem, i) => {
      originalContent.highlighters[i] = elem.innerHTML;
    });
    ...
    // Split text for highlighter elements (already declared above when storing original content)
  ...
  // Video ScrollTrigger setup - works for all videos
  function setupVideoScrollTriggers() {
    ...
    console.log(`Found ${videos.length} total videos on page`);

    // Create ScrollTriggers for each video
    videos.forEach((video, index) => {
      ScrollTrigger.create({
        ...
        id: `cs-video-${video.id || index}`
      });
      console.log(`✅ ScrollTrigger created for video: ${video.id || 'video-' + index}`);
    });
  }
  ...
  function cleanupSplitText() {
    ...
    // Restore highlighter elements
    const highlighters = document.querySelectorAll('.ux_text_highlighter_deco');
    highlighters.forEach((elem, i) => {
      if (elem && originalContent.highlighters && originalContent.highlighters[i]) {
        elem.innerHTML = originalContent.highlighters[i];
      }
    });
  }
