...
function ux_home_v2() {
  ...
  function init() {
    console.log('🔵 ux_home_v2.js init called at:', new Date().toISOString());
    ...
    // mm.add("(min-width: 769px)", () => {
      ...
      console.log('ux_nav_link_menu element:', ux_nav_link_menu);
      console.log('ux_nav_link_menu parent:', ux_nav_link_menu?.parentElement);
      ...
      console.log('🔸 Creating SplitText for HOME, innerHTML before:', ux_nav_link_home?.innerHTML);
      let split_ux_nav_link_home = new SplitText(ux_nav_link_home, {type:"chars, words"});
      console.log('🔸 HOME split done, chars:', split_ux_nav_link_home.chars);

      console.log('🔸 Creating SplitText for MENU, innerHTML before:', ux_nav_link_menu?.innerHTML);
      let split_ux_nav_link_menu = new SplitText(ux_nav_link_menu, {type:"chars, words"});
      console.log('🔸 MENU split done, chars:', split_ux_nav_link_menu.chars);
      ...
      let banner_text_tl = gsap.timeline({
        onStart: () => console.log('🎬 Banner text timeline STARTED'),
        onComplete: () => console.log('✅ Banner text timeline COMPLETED')
      });
      banner_text_tl
        ...
        .slideUp(split_ux_nav_link_menu.chars, {}, "banner_text_tl_start+=0.2")
      ...
      ap_grid_container_ux_home_cs.forEach((element, index) => {
        ...
        const ux_home_cs_animation = () => {
          ...
          .slideUp(ux_card_company_name_text, {}, "<")
  ...
  const initializeOnce = () => {
    ...
    // Wait for both fonts AND window.load to avoid sj_menu cloning during animation
    Promise.all([
      ...
      console.log('✅ Fonts loaded AND window.load complete');
      console.log('🎬 Starting animations now (after sj_menu initialization)!');
      init();
